...
this.UserGroup1 = this.fb1.group({
  ...
  if (usuario.contrasenia == usuario.confirmarContrasena) {
    ...
    const params = {
      ...
      id_lote: usuario.id_lote,
      

      //  Intercomunicador: 123,
      //  Codigo_acceso: "123"
    };
    this.id_lote1=usuario.id_lote;
    console.log(params)
...
delete_user(id_usuario_lote: any){
  ...
  id_lote1:number|undefined ;
  generarInvitacion(correoElectronico: string, idFraccionamiento: number): void {
    const token = uuidv4();
    this.invitacionService.generarInvitacion(token,correoElectronico, this.dataService.obtener_usuario(1),500,"cracatoa",this.dataService.obtener_usuario(5), "arrendatario" )
      .subscribe(
        response => {
